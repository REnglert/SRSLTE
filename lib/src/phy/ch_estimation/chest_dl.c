                         ...
                         uint32_t               rxant_id)
{
  ...
  //  FILE *fp1 = fopen("pilot_estimates.txt", "a++");
  //  FILE *fp2 = fopen("csr_refs.txt", "a++");
  FILE *fp3 = fopen("layout.txt", "a++");
  FILE *fp4 = fopen("chest_dl.csv", "a++");
  
  //Write time
  fprintf(fp3, "TTI, real_pilot_estiamte, imag_pilot_estimate, real_csr_ref, imag_csr_ref \n");
  fclose(fp3);
  
  //For each npilot estimate write pilot_estiamte and scr_ref
  for(int i = 0; i < npilots; i++){
    fprintf(fp4, "%d,%g,%g,%g,%g\n", sf->tti, creal(q->pilot_estimates[i]), cimag(q->pilot_estimates[i]), creal(q->csr_refs.pilots[port_id / 2][sf->tti % 10][i]), cimag(q->csr_refs.pilots[port_id / 2][sf->tti % 10][i]));
